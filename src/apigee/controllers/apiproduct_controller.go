...
func (r *ApiProductReconciler) Reconcile(req ctrl.Request) (ctrl.Result, error) {
	...
	if err := r.Client.Get(context.TODO(), types.NamespacedName{Name: "apigee-config", Namespace: "apigee-system"}, &configMap); err != nil && apierrs.IsNotFound(err) {
...
func parseInputAndCreateJSON(instance apigeev1.ApiProduct, log logr.Logger) string {
	...
	//annotation := instance.ObjectMeta.Annotations["kubectl.kubernetes.io/last-applied-configuration"]
	//log.V(0).Info(annotation)
